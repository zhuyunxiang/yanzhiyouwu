...
function log_write($arg, $file = '', $line = '') {
    ...
    file_put_contents(DATA_DIR  . '/log.txt', $str);
}
 ...
 */
// for sae 改造
function make_dir($folder, $sae="") {
    $reval = 0;
//    dump($reval, "reval is dir");
    if (!file_exists($folder)) {
        /* 如果目录不存在则尝试创建该目录 */
        @umask(0);
        $folder = str_replace($sae, "", $folder);
        /* 将目录路径拆分成数组 */
...
//            dump(file_put_contents($base . "/index.htm", '-'),'make default html');
//            dump($reval, 'html back reval');
        }
    } else {
        /* 路径已经存在。返回该路径是不是一个目录 */
...
function move_upload_file($file_name, $target_name = '') {
    if (function_exists("move_uploaded_file")) {
        if (move_uploaded_file($file_name, saestor($target_name))) {
            ...
            return true;
        } else if (copy($file_name, saestor($target_name))) {
            @chmod($target_name, 0755);
            return true;
        }
...
//        dump($result[$cache_name],$cache_file_path);
        return $result[$cache_name];
    } else {
        return false;
    }
