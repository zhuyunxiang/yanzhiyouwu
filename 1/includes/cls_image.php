...
class cls_image {
    ...
    function upload_image($upload, $dir = '', $img_name = '') {
        ...
        if ($this->move_file($upload, $img_name)) {
            //dump($img_name,'img_name cls_image 104');
//            sleep(0.5);
            return $img_name;
        } else {
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='') {
        ...
        //dump($filename, 'filename 232');
        //dump($img_thumb, '$img_thumb 235');
        ob_start();
        /* 生成文件 */
        if (function_exists('imagejpeg')) {
            ...
            //$s->write('upload', $dst_path, $imgstr);
            file_put_contents($dir . $filename, $imgstr);
            ob_end_clean();
            ...
            $this->error_msg = $GLOBALS['_LANG']['creating_failure'] . "260";
            $this->error_no = ERR_NO_GD;
            return false;
        }
...
//         if (file_exists($dir . $filename)) {
        return $dir . $filename;
//        } else {
////            dump($dir . $filename, 'dir filename 276');
//            $this->error_msg = $GLOBALS['_LANG']['writting_failure'] . "276";
//            $this->error_no = ERR_DIRECTORY_READONLY;
        //exit();
//            return false;
//        } 
...
//        dump($target, "target");
        switch ($source_info[2]) {
            case 'image/gif':
            case 1:
                imagegif($source_handle);
                $imgstr = ob_get_contents();
                file_put_contents($target, $imgstr);
                ob_end_clean();
                break;
            ...
            default:
                $this->error_msg = $GLOBALS['_LANG']['creating_failure'] . "402";
                $this->error_no = ERR_NO_GD;
...
//        $path = realpath($target);
        ...
        if ($path) {
            ...
            $this->error_msg = $GLOBALS['_LANG']['writting_failure'] . "416";
            $this->error_no = ERR_DIRECTORY_READONLY;
