<?php
    ...
    function API_AddGoods($post)
    {
        ...
        if(!empty($_POST['goods_img']['Data']))
        {
            ...
            /* 图片属性 */
            $img_property = ($image->gd_version() > 0)?getimagesize(  $goods_img):array();
       ...
       if($catgory_id == '' && $new_cat_name != '')
        ...
        else
        {
            ...
            {
                if (is_file($row['goods_img']))
                {
                    @unlink($row['goods_img']);
                }
                @rename( $goods_img, $row['goods_img']);
                if (is_file($row['original_img']))
                {
                    @unlink($row['original_img']);
                }
                @rename($original_img, $row['original_img']);
            }

            if ($goods_thumb && $row['goods_thumb'])
            {
                if (is_file($row['goods_thumb']))
                {
                    @unlink($row['goods_thumb']);
                }
                @rename($goods_thumb, $row['goods_thumb']);
            }
